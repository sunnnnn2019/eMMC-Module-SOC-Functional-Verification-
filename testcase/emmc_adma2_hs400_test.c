...
//NORMAL_INT_STAT_EN_R
  ...
  emmc_wio_8(0x29,0xe);
  //HOST_CTRL1_R
  //emmc_wio_8(0x28,0x10);  //1 bit mode
  //emmc_wio_8(0x28,0x12);  // 4 bits mode
  emmc_wio_8(0x28,0x30);    // 8 bits mode
...
//  send_emmc_cmd(0, 0, 0, 0, 0, 0, 0, 0xf0f0f0f0);//cmd_index,cmd_type,data_present_sel,idx_check,crc_check,sub_cmd,resp_type,argument
  ...
  //HS400
  a_printf("start hs400 config\n",i);
  //Host changes frequency to 25MHz
  emmc_wio_16(0x2c,0x40f);//CLK_CTRL_R
  
  //Set HS_TIMING to 0x01(High Speed)
  send_emmc_cmd(6, 0, 0, 0, 0, 0, 0x2, 0x3b90104);//cmd_index,cmd_type,data_present_sel,idx_check,crc_check,sub_cmd, resp_type,argument
  
  //Set the bus width to DDR8Bit (CMD6)
  send_emmc_cmd(6, 0, 0, 0, 0, 0, 0x2, 0x3b70604);//cmd_index,cmd_type,data_present_sel,idx_check,crc_check,sub_cmd, resp_type,argument
  
  //Set HS_TIMING to 0x3(HS400)
  send_emmc_cmd(6, 0, 0, 0, 0, 0, 0x2, 0x3b90304);//cmd_index,cmd_type,data_present_sel,idx_check,crc_check,sub_cmd, resp_type,argument
  
  //Host may changes frequency to \u2264 200MHz
  emmc_wio_16(0x2c,0x00f);//CLK_CTRL_R

  //HOST_CTRL2_R
  emmc_rio_16(0x3e,&rdata_16);
  rdata_16 = (rdata_16 & 0xfff8)|0x7;
  emmc_wio_16(0x3e,rdata_16);

  //DLLDL_CNFG
  emmc_rio_8(0x328,&rdata_8);
  rdata_8 = (rdata_8 & 0x9f)|0x60;
  emmc_wio_8(0x328,rdata_8);

  //DLL_OFFST
  emmc_wio_8(0x329,16);

  //DLL_CTRL
  emmc_rio_8(0x324,&rdata_8);
  rdata_8 = (rdata_8 & 0xfd)|0x2;
  emmc_wio_8(0x324,rdata_8);

  ////AT_CTRL_R
  //emmc_rio_32(0x540,&rdata_32);
  //rdata_32 = (rdata_32 & 0xffffffef) | 0x10;
  //emmc_wio_32(0x540,rdata_32);

  ////AT_STAT_R
  //emmc_rio_32(0x544,&rdata_32);
  //rdata_32 = (rdata_32 & 0xffffff00) | 16;
  //emmc_wio_32(0x544,rdata_32);
  ...
  //block_count = 2;
  block_count = 1;
  wr_data_addr = (unsigned int *)0x82000000;
  a_printf("start adma2 write\n",i);
  //descriptor
  *(unsigned int *) (0x81000000      ) = 0x23 | ((block_len*block_count) << 16);
  *(unsigned int *) (0x81000000 + 0x4) = (unsigned int) wr_data_addr;
  *(unsigned int *) (0x81000000 + 0x8) = 0;
  *(unsigned int *) (0x81000000 + 0xc) = 0;
  //data
  srand(769);
  for(i=0;i<block_len*block_count/4;i=i+1){
      *(unsigned int *) (wr_data_addr + i)    = rand();
  }

  emmc_wio_32(0x58,0x81000000);//ADMA_SA_LOW_R
  emmc_wio_32(0x5c,0x0);//ADMA_SA_HIGH_R
  emmc_wio_32(0x0,block_count);//Block Count
  emmc_wio_16(0x4,0x7000|block_len);//BLOCKSIZE_R,data byte length of one block
  //emmc_wio_16(0xc,0x2f);//XFER_MODE_R
  emmc_wio_16(0xc,0x0f);//XFER_MODE_R

  //CMD25,  WRITE_MULTIPLE_BLOCK
  //send_emmc_cmd(25, 0, 1, 1, 0, 0, 0x2, 0x100);//cmd_index,cmd_type,data_present_sel,idx_check,crc_check,sub_cmd, resp_type,argument
  //CMD24,  WRITE_BLOCK
  send_emmc_cmd(24, 0, 1, 1, 0, 0, 0x2, 0x100);//cmd_index,cmd_type,data_present_sel,idx_check,crc_check,sub_cmd, resp_type,argument

  emmc_rio_16_expect(0x30,0x2,0x2,200);    //NORMAL_INT_STAT_R
  emmc_rio_16(0x32,&rdata_16);    //ERROR_INT_STAT_R
  if(rdata_16 != 0){
    a_printf("error code is 0x%x\n",rdata_16);
  }
  emmc_rio_16(0x3c,&rdata_16);//AUTO_CMD_STAT_R

  //ADMA Read
  rd_data_addr = (unsigned int *)0x82100000;
  a_printf("start adma2 read\n",i);
  //descriptor
  *(unsigned int *) (0x81000010      ) = 0x23 | ((block_len*block_count) << 16);
  *(unsigned int *) (0x81000010 + 0x4) = (unsigned int) rd_data_addr;
  *(unsigned int *) (0x81000010 + 0x8) = 0;
  *(unsigned int *) (0x81000010 + 0xc) = 0;


  emmc_wio_32(0x58,0x81000010);//ADMA_SA_LOW_R
  emmc_wio_32(0x5c,0x0);//ADMA_SA_HIGH_R
  emmc_wio_32(0x0,block_count);//Block Count
  emmc_wio_16(0x4,0x7000|block_len);//BLOCKSIZE_R,data byte length of one block
  //emmc_wio_16(0xc,0x3f);//XFER_MODE_R
  emmc_wio_16(0xc,0x1f);//XFER_MODE_R

  //CMD18,  READ_MULTIPLE_BLOCK
  //send_emmc_cmd(18, 0, 1, 1, 0, 0, 0x2, 0x100);//cmd_index,cmd_type,data_present_sel,idx_check,crc_check,sub_cmd, resp_type,argument
  //CMD17,  READ_BLOCK
  send_emmc_cmd(17, 0, 1, 1, 0, 0, 0x2, 0x100);//cmd_index,cmd_type,data_present_sel,idx_check,crc_check,sub_cmd, resp_type,argument
  ...
  for(i=0;i<20;i=i+1){
      a_printf("wait %d of 20...\n",i);
  }
  //data check
  error_flag = 0;
  a_printf("start data check\n");
  for(i=0;i<block_len*block_count/4;i=i+1){
      if(*(unsigned int *)rd_data_addr != *(unsigned int *)wr_data_addr){
          error_flag = 1;
          a_printf("data check error: dma read addr:0x%x value 0x%08x, dma write addr:0x%x value 0x%08x\n",(unsigned int)rd_data_addr, *(unsigned int *)rd_data_addr,(unsigned int)wr_data_addr, *(unsigned int *)wr_data_addr);
      }else{
          //a_printf("data check addr:0x%x ok\n",(unsigned int)rd_data_addr);
      }
      rd_data_addr += 1;
      wr_data_addr += 1;
  }
  if(error_flag == 0){
      a_printf("data check ok\n");
  }
